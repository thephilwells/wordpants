...
function handleSubmit() {
  ...
  if (winScreen) {
    ...
    if (checkForEnoughSquiggle()) {
      new Audio('/sfx/whistleup.wav').play()
      pullUpPants()
    } else {
      shockJean()
...
function handleRestart() {
  new Audio('/sfx/whistledown.wav').play()
  squigglePixels = []
  context.clearRect(0, 0, canvas.width, canvas.height)
  context.closePath()
  redrawHeaderTemplate(headerCanvas)
...
async function pullUpPants() {
  ...
  for (let i = 0; i < squigglePixels.length; i++) {
    ...
    const newPixel = { x: initialPixels.x, y: initialPixels.y - 1 }
  ...
  lowestPixelValue--
  console.log(
    `WAISTLINE: ${waistline} HIGHEST SQUIGGLE POINT: ${highestPixelValue}`
  )
